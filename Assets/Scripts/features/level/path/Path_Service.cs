...
namespace td.features.level
{
    public class Path_Service
    {
        ...
        public void InitPath()
        {
            ...
            for (var idx = 0; idx < levelState.GetSpawnCount(); idx++)
            {
                ref var spawnCell = ref levelState.GetSpawnByIndex(idx);
                // ref var spawnCell = ref levelState.GetSpawn(idx);
                if (spawnCell.type != CellTypes.CanWalk) continue; //todo error
                // if (!spawnCell.HasValue || !levelMap.HasCell(spawnCell.Value.x, spawnCell.Value.y, CellTypes.CanWalk)) continue;
                // ref var spawnCell = ref levelMap.GetCell(spawnCell.Value.x, spawnCell.Value.y, CellTypes.CanWalk);
                // spawnCell.spawnNumber = (byte)idx;
        ...
        {
            if (!levelState.HasCell(coords.x, coords.y)) return;

            ref var cell = ref levelState.GetCell(coords.x, coords.y);
            ...
            foreach (var direction in directions)
            {
                ...
                if (!levelState.HasCell(nCoords.x, nCoords.y, CellTypes.CanWalk)) continue;
                ref var nextCell = ref levelState.GetCell(nCoords.x, nCoords.y, CellTypes.CanWalk);
        ...
        private void ClearBadPath(ref Cell cell)
        {
            ...
            if (!levelState.HasCell(prevCoords, CellTypes.CanWalk)) return;
                
            ref var prevCell = ref levelState.GetCell(prevCoords.x, prevCoords.y, CellTypes.CanWalk);
