...
namespace td.features.shards
{
    public class ShardDragNDropSystem : IEcsRunSystem, IEcsInitSystem
    {
        ...
        private readonly
            EcsFilterInject<Inc<Shard, DragEndEvent, DraggingStartedData, Ref<GameObject>>,
                Exc<IsDestroyed, IsRollbackDragging>> dragEndEventEntities = default;

        private readonly EcsFilterInject<Inc<Shard, DragRollbackEvent>, Exc<IsDestroyed, IsRollbackDragging>>
            rollbackEntities = default;

        private readonly EcsFilterInject<Inc<Shard, IsDragging, DraggingStartedData, Ref<GameObject>>, Exc<IsDestroyed>>
            draggableEntities = default;

        private readonly
            EcsFilterInject<Inc<Shard, ShardInCollection, IsHidden, Ref<GameObject>>, Exc<IsDestroyed, IsDisabled>>
            hiddenEntities = default;
        ...
        private void DragStart(IEcsSystems systems)
        {
            foreach (var downEventEntity in downEventEntities.Value)
            {
                    ...
                    shared.draggableShardPackedEntity.Unpack(world, out var draggableShardEntity))
                {
                    ...
                    //todo add timeout ~500ms before start drag, for simple click for show information
        ...
        private void Draging(IEcsSystems systems)
        {
            ...
            {
                var positionOnCanvasCamera =
                    draggableEntities.Pools.Inc4.Get(draggableEntity).reference.transform.position;
                var positionOnViewport = shared.canvasCamera.WorldToViewportPoint(positionOnCanvasCamera);
                positionOnViewport.z = 0f;
                var positionOnMainCamera = shared.mainCamera.ViewportToWorldPoint(positionOnViewport);
                positionOnMainCamera.z = 0f;

                // Debug.Log("positionOnCanvasCamera = " + positionOnCanvasCamera);
                // Debug.Log("positionOnViewport = " + positionOnViewport);
                // Debug.Log("positionOnMainCamera = " + positionOnMainCamera);
                ...
                var cell = levelMap.GetCell(
                    positionOnMainCamera,
                    CellTypes.CanBuild
                );
                // Debug.Log("cell = " + cell);

                if (ShardUtils.HasHoveredShard(world))
                {
                    canDrop = true;

                    // todo calculate cost
                    // todo check money
                    shared.combineShardCostPopup.ShowCombineCost(100, true);
                }
                else if (cell != null && cell.HasBuilding<ShardTower>(world))
                {
                    ref var shardTower = ref cell.GetBuilding<ShardTower>(world, out var towerEntity);
                    // todo check money to combine shard with shardInTower
                    canDrop = true;
                    // todo calculate cost
                    // todo check money
                    ...
                    {
                        shared.combineShardCostPopup.ShowIntegrateCost(20, true);
                    }
                }
                ...
                else
                {
                    ...
                    shared.combineShardCostPopup.Hide();
                }
            }
        }
        ...
        private void DragEnd(IEcsSystems systems)
        {
            foreach (var draggableEntity in dragEndEventEntities.Value)
            {
                ...
                var vp = shared.canvasCamera.WorldToViewportPoint(shardScreenPosition);
                var shardPosition = shared.mainCamera.ViewportToWorldPoint(vp);

                bool dropped;

                if (ShardUtils.HasHoveredShard(world))
                {
                    ref var hoveredShard = ref ShardUtils.GetHoveredShard(world, out var hoveredShardEntity);
                ...
                else
                {
                    ...
                    target.resetAnchoredPositionZ = true;
                }
        ...
        private bool DropToShard(ref Shard targetShard, ref Shard sourceShard, int targetEntity)
        {
            ...
            world.DelComponent<ShardColor>(targetEntity);
        ...
        {
            shardPrefab = prefabService.GetPrefab(PrefabCategory.Shard, "Shard");
        }
    }
}