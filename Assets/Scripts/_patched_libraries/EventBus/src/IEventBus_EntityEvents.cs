...
namespace Leopotam.EcsLite
{
	...
	public interface IEventBus_EntityEvents : IEventBus_Subsystem
	{
		...
		delegate void EntityEventActionRef<T>(EcsPackedEntityWithWorld packedEntity, ref T item);
		...
		ref T Add<T>(int targetEntity, EcsWorld targetEntityWorld, EcsPool<T> optionalCachedPool = default)
			where T : struct, IEventEntity;
		...
		void ListenTo<T>(EntityEventActionRef<T> whenEventFired) where T : struct, IEventEntity;
		...
		IDisposable SubscribeTo<T>(EntityEventActionRef<T> whenEventFired) where T : struct, IEventEntity;
		...
		void RemoveListener<T>(EntityEventActionRef<T> whenEventFired) where T : struct, IEventEntity;
		...
		bool Has<T>(EcsPool<T> cachedPool = default)
			where T : struct, IEventEntity;
		// bool Has<T>(EcsPackedEntityWithWorld targetEntity, EcsPool<T> optionalCachedPool = default)
			// where T : struct, IEventEntity;
		// bool Has<T>(int targetEntity, EcsWorld targetEntityWorld, EcsPool<T> optionalCachedPool = default)
			// where T : struct, IEventEntity;

		EcsFilter GetFilter<T>() where T : struct, IEventEntity;
		EcsPool<T> GetPool<T>() where T : struct, IEventEntity;
	}
}