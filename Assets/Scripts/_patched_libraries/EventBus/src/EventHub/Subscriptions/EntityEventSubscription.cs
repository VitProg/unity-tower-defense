...
namespace Leopotam.EcsLite
{
	...
	internal class EntityEventSubscription<T> : IEntityEventSubscription
	{
		...
		private readonly List<IEventBus_EntityEvents.EntityEventActionRef<T>>
			_subscriptionsGlobal = new();
		...
		public void Invoke(EcsPackedEntityWithWorld source, IEventEntity evt)
		{
			if (evt is T castedEvent)
			{
				...
				foreach (var action in _subscriptionsGlobal)
					action.Invoke(source, ref castedEvent);
			}
			else
			{
		...
		public void ListenTo(IEventBus_EntityEvents.EntityEventActionRef<T> whenEventFired)
		{
			_subscriptionsGlobal.Add(whenEventFired);
		}
		...
		public IDisposable SubscribeTo(IEventBus_EntityEvents.EntityEventActionRef<T> whenEventFired)
		{
			_subscriptionsGlobal.Add(whenEventFired);
		...
		public void RemoveListener(IEventBus_EntityEvents.EntityEventActionRef<T> whenEventFired)
		{
			_subscriptionsGlobal.Remove(whenEventFired);
		}
		...
		private class UnsubscribeRecord : Pooled<UnsubscribeRecord>, IDisposable
		{
			...
			internal UnsubscribeRecord Assign(EntityEventSubscription<T> sub, IEventBus_EntityEvents.EntityEventActionRef<T> action)
			{
				_sub = sub;
				_action = action;
				return this;
