...
namespace Leopotam.EcsLite
{
	public interface IEventBus_FlagComponents : IEventBus_Subsystem
	{
		...
		void Del<T>(EcsPackedEntityWithWorld targetEntity, EcsPool<T> optionalCachedPool = default)
			where T : struct, IFlagComponent;
		
		bool Has<T>(EcsPackedEntityWithWorld targetEntity, EcsPool<T> optionalCachedPool = default)
			where T : struct, IFlagComponent;
		
		ref T Add<T>(int targetEntity, EcsWorld world, EcsPool<T> optionalCachedPool = default)
			where T : struct, IFlagComponent;

		void Del<T>(int targetEntity, EcsWorld world, EcsPool<T> optionalCachedPool = default)
			where T : struct, IFlagComponent;

		bool Has<T>(int targetEntity, EcsWorld world, EcsPool<T> optionalCachedPool = default)
			where T : struct, IFlagComponent;
		...
		/// </param>
		/// <typeparam name="T">The flag component type.  Must derive from IFlagComponent.</typeparam>
		ref T Add<T>(int targetEntity, EcsPool<T> cachedPool) where T : struct, IFlagComponent;

		void Del<T>(int targetEntity, EcsPool<T> cachedPool) where T : struct, IFlagComponent;

		bool Has<T>(int targetEntity, EcsPool<T> cachedPool) where T : struct, IFlagComponent;
