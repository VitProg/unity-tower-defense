...
namespace Leopotam.EcsLite
{
	...
	public class BasicServiceContainer : IServiceContainer
	{
		...
		{
			var type = typeof(T);
			return Add(type, serviceInstance);
		}
		...
		{
			if (Services.ContainsKey(registrationType))
				throw new DuplicateNameException($"Service for {registrationType.Name} already defined");

			Services.Add(registrationType, serviceInstance);
			return this;
		}
		...
		public void Remove<T>()
		{
			...
			foreach (var removeType in toRemove) Services.Remove(removeType);
		}
